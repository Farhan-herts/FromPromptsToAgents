...
DISH_PROFILES = {
    "pasta carbonara": {
        "estimated_cost_usd_total": 10.40,
        "nutrition_per_serving": {"protein_g": 29, "calories_kcal": 690, "fat_g": 25, "carbs_g": 72},
    },
    "souffle": {
        "estimated_cost_usd_total": 12.50,
        "nutrition_per_serving": {"protein_g": 18, "calories_kcal": 410, "fat_g": 24, "carbs_g": 18},
    },
    "sushi rolls": {
        "estimated_cost_usd_total": 16.80,
        "nutrition_per_serving": {"protein_g": 24, "calories_kcal": 520, "fat_g": 14, "carbs_g": 66},
    },
    "pizza margherita": {
        "estimated_cost_usd_total": 9.60,
        "nutrition_per_serving": {"protein_g": 18, "calories_kcal": 610, "fat_g": 18, "carbs_g": 86},
    },
    "beef stir-fry": {
        "estimated_cost_usd_total": 14.20,
        "nutrition_per_serving": {"protein_g": 34, "calories_kcal": 430, "fat_g": 17, "carbs_g": 28},
    },
    "chocolate cake": {
        "estimated_cost_usd_total": 11.80,
        "nutrition_per_serving": {"protein_g": 6, "calories_kcal": 450, "fat_g": 20, "carbs_g": 62},
    },
    "fish and chips": {
        "estimated_cost_usd_total": 17.20,
        "nutrition_per_serving": {"protein_g": 36, "calories_kcal": 780, "fat_g": 28, "carbs_g": 82},
    },
    "pad thai": {
        "estimated_cost_usd_total": 13.60,
        "nutrition_per_serving": {"protein_g": 22, "calories_kcal": 540, "fat_g": 19, "carbs_g": 63},
    },
    "french omelette": {
        "estimated_cost_usd_total": 6.80,
        "nutrition_per_serving": {"protein_g": 19, "calories_kcal": 320, "fat_g": 25, "carbs_g": 3},
    },
    "bread": {
        "estimated_cost_usd_total": 4.90,
        "nutrition_per_serving": {"protein_g": 8, "calories_kcal": 210, "fat_g": 1, "carbs_g": 42},
    },
}


# ---------------------------------------------------------------------------
# Helper utilities for the budget + nutrition challenge
# ---------------------------------------------------------------------------
...
def _resolve_dish(dish_name: str):
    ...
    for db_key, db_dish in DISHES.items():
        searchable_parts = [
            db_key,
            db_dish.get("name", "").lower(),
            db_dish.get("cuisine", "").lower(),
            " ".join(ingredient["item"] for ingredient in db_dish.get("ingredients", [])),
        ]
        searchable = " | ".join(searchable_parts)
        if key and (key in searchable or searchable in key):
            return db_key, db_dish
...
def _scale_quantity(quantity: str, factor: float) -> str:
    text = quantity.strip()
    lowered = text.lower()
    ...
    if any(token in lowered for token in ["to taste", "as needed", "handful", "pinch", "none"]):
        return text

    mult_match = re.match(r"^(\d+(?:\.\d+)?)\s*x\s*(\d+(?:\.\d+)?)\s*([a-zA-Z]+)$", lowered)
    if mult_match:
        count = float(mult_match.group(1)) * factor
        size = float(mult_match.group(2))
        unit = mult_match.group(3)
        return f"{_format_number(count)} x {_format_number(size)}{unit}"

    simple_match = re.match(r"^(\d+(?:\.\d+)?)\s*([a-zA-Z°]+)$", text)
    if simple_match:
        value = float(simple_match.group(1)) * factor
        unit = simple_match.group(2)
        return f"{_format_number(value)}{unit}"

    bare_number = re.match(r"^(\d+(?:\.\d+)?)$", text)
    if bare_number:
        value = float(bare_number.group(1)) * factor
        return _format_number(value)

    spaced_match = re.match(r"^(\d+(?:\.\d+)?)\s+(.+)$", text)
    if spaced_match:
        value = float(spaced_match.group(1)) * factor
...
def _build_dish_analysis(canonical_key: str, dish: dict, requested_servings: int) -> dict:
    profile = DISH_PROFILES.get(canonical_key, {})
    base_servings = int(dish.get("servings", requested_servings or 1) or 1)
    requested_servings = int(requested_servings or base_servings)
    factor = requested_servings / base_servings

    scaled = deepcopy(dish)
    scaled["base_servings"] = base_servings
    scaled["requested_servings"] = requested_servings
    scaled["scaling_factor"] = round(factor, 4)
    scaled["ingredients"] = [
        {
            **ingredient,
    ...
    nutrition_per_serving = profile.get("nutrition_per_serving", {})
    scaled["nutrition_per_serving"] = nutrition_per_serving
    scaled["nutrition_total"] = {
        metric: round(value * requested_servings, 2)
        for metric, value in nutrition_per_serving.items()
    }

    estimated_cost_total = round(profile.get("estimated_cost_usd_total", 0.0) * factor, 2)
    scaled["estimated_cost_usd_total"] = estimated_cost_total
    scaled["estimated_cost_usd_per_serving"] = round(
        estimated_cost_total / requested_servings, 2
    ) if requested_servings else 0.0

    return scaled



def _request_match_score(canonical_key: str, dish: dict, user_request: str, preferred_dish: str) -> int:
    score = 0
    searchable = " ".join(
        [
            canonical_key,
            dish.get("name", ""),
            dish.get("cuisine", ""),
            " ".join(ingredient["item"] for ingredient in dish.get("ingredients", [])),
        ]
    ).lower()

    request_tokens = re.findall(r"[a-zA-Z]+", _normalise_text(user_request))
    for token in request_tokens:
        if token in searchable:
            score += 3

    preferred_key = _normalise_text(preferred_dish)
    if preferred_key and preferred_key in searchable:
        score += 25
...
def _build_candidate(
    ...
    servings: int,
    min_protein_g: float,
    max_calories_kcal: float,
) -> dict:
    analysis = _build_dish_analysis(canonical_key, dish, servings)
    protein = analysis.get("nutrition_per_serving", {}).get("protein_g", 0)
    calories = analysis.get("nutrition_per_serving", {}).get("calories_kcal", 0)
    total_cost = analysis.get("estimated_cost_usd_total", 0)

    violations = []
    if budget_usd > 0 and total_cost > budget_usd:
        violations.append(f"Over budget by ${round(total_cost - budget_usd, 2):.2f}")
    if min_protein_g > 0 and protein < min_protein_g:
        violations.append(f"Protein short by {round(min_protein_g - protein, 1):.1f} g per serving")
    if max_calories_kcal > 0 and calories > max_calories_kcal:
        violations.append(f"Calories exceed cap by {round(calories - max_calories_kcal, 1):.1f} kcal per serving")

    fits_budget = budget_usd <= 0 or total_cost <= budget_usd
    fits_protein = min_protein_g <= 0 or protein >= min_protein_g
    fits_calories = max_calories_kcal <= 0 or calories <= max_calories_kcal
    fits_all = fits_budget and fits_protein and fits_calories

    score = _request_match_score(canonical_key, dish, user_request, preferred_dish)
    if fits_all:
        score += 50
    if fits_budget:
        score += 8
    ...
    if max_calories_kcal > 0:
        score -= int(abs(calories - max_calories_kcal) / 40)

    reasons = []
    if _request_match_score(canonical_key, dish, user_request, preferred_dish) > 0:
        reasons.append("Matches the requested meal style or keywords")
    if fits_budget:
        reasons.append("Fits the stated budget")
    if fits_protein:
        reasons.append("Meets the protein target")
    if fits_calories:
        reasons.append("Stays within the calorie cap")
    if not reasons:
        reasons.append("Closest available option in the current recipe database")

    return {
        "dish_key": canonical_key,
        "dish_name": dish["name"],
        ...
        "cost_per_serving_usd": analysis.get("estimated_cost_usd_per_serving", 0),
        "protein_per_serving_g": protein,
        "calories_per_serving_kcal": calories,
        "fits_budget": fits_budget,
        "fits_protein": fits_protein,
        "fits_calories": fits_calories,
        "fits_all_constraints": fits_all,
...
def find_matching_dishes(
    ...
    """
    Shortlist dishes that best fit the user's meal request, budget, servings, and
    nutrition constraints. This is the main planning tool for the challenge.

    Args:
        user_request: Free-text meal request such as 'high protein pasta'
        budget_usd: Maximum budget for the full recipe
        servings: Desired number of servings
        min_protein_g: Minimum protein required per serving
        max_calories_kcal: Maximum calories allowed per serving. Use 0 for no cap.
        preferred_dish: Optional preferred dish to prioritise when it fits
    """
    servings = max(int(servings or 1), 1)
    budget_usd = float(budget_usd or 0)
    min_protein_g = float(min_protein_g or 0)
    max_calories_kcal = float(max_calories_kcal or 0)

    candidates = [
        _build_candidate(
            canonical_key=db_key,
            dish=dish,
        ...
        for db_key, dish in DISHES.items()
    ]

    candidates.sort(
        key=lambda item: (
            item["fits_all_constraints"],
            item["score"],
    ...
    strict_matches = [candidate for candidate in candidates if candidate["fits_all_constraints"]]
    ...
    result = {
        ...
        "candidate_dishes": candidates[:5],
    }
    return json.dumps(result, indent=2)


@mcp.tool()
def analyse_dish(dish_name: str, servings: int = 4) -> str:
    """
    Analyse a dish and return structured information including scaled ingredients,
    techniques, equipment, temperatures, step-by-step instructions, estimated cost,
    and nutrition summary.

    Args:
        dish_name: Name of the dish to analyse (e.g. 'pasta carbonara', 'souffle')
        servings: Desired number of servings for scaling the recipe
    """
    canonical_key, dish = _resolve_dish(dish_name)
    ...
    return json.dumps(
        _build_dish_analysis(canonical_key, dish, int(servings or dish.get("servings", 1))),
        indent=2,
    )
